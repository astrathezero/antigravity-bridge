...
| agy tool use | **Blocked** | API mode: the model is told not to use agy's own tools; if agy still starts a tool step the run is killed within a second and the request fails with a clear error. `ANTIGRAVITY_ALLOW_CLI_TOOLS=1` re-enables agentic runs. |
