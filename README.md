...
## ⚡ [NEW] Antigravity Bridge (Node.js Edition) - Now Available on `feat/nodejs-version` 🚀

> [!NOTE]
> ### 🧪 Public Beta & Transition Roadmap (สถานะ Beta และแผนการควบรวม)
> - **Current Status (Parallel Testing Beta):** The Node.js Edition is currently in **Public Beta** on branch [`feat/nodejs-version`](https://github.com/astrathezero/antigravity-bridge/tree/feat/nodejs-version) running on default port **`8008`** specifically designed for **parallel testing** alongside the established Python version (`8000`).
> - **Migration Plan (แผนการควบรวม):** Once the Node.js version is thoroughly battle-tested and confirmed 100% stable under heavy production load, **it will be merged into `main` to completely replace the Python codebase**, standardizing Antigravity Bridge solely on Node.js.
> - *(ภาษาไทย: เวอร์ชัน Node.js อยู่ในสถานะ **Beta** ออกแบบมาเพื่อให้ผู้ใช้งานทดสอบรันควบคู่ (Parallel Testing) ไปกับเวอร์ชัน Python เดิมบนพอร์ต 8008 โดยไม่ชนกัน หากผลการทดสอบนิ่งและมีเสถียรภาพสมบูรณ์แล้ว จะทำการ merge รวมเข้าเป็นเวอร์ชันหลัก และเปลี่ยนผ่านไปใช้ Node.js เพียงเวอร์ชันเดียวแบบเบ็ดเสร็จ)*

### 🌟 Why Choose the Node.js Edition over Python? (ข้อดีที่เหนือกว่าเวอร์ชัน Python)

| Feature / Metric | Python Version (`main`) | Node.js Version (`feat/nodejs-version`) | Advantage |
| :--- | :--- | :--- | :--- |
| **Default Port** | `8000` | **`8008`** | 🟢 **Zero Port Collision:** Can run side-by-side with Python without port conflicts |
| **Concurrency Model** | Multi-threading / Blocking GIL | **Non-blocking Event Loop (`libuv`)** | ⚡ Much higher concurrent throughput; never blocks during deep reasoning |
| **Dependencies** | Python 3.8+ Standard Library | **Zero External Dependencies (100% Native ESM)** | 📦 No `npm install`, no express, runs out-of-the-box on Node 18+ |
| **Memory Footprint** | ~80 - 150 MB | **~35 - 55 MB** | 🍃 Highly lightweight, ideal for servers & background services |
| **Cold Start Time** | ~150 - 300 ms | **< 25 ms** | 🚀 Near-instantaneous process startup |
| **Code Architecture** | Monolithic file (6,100+ lines) | **Modular Architecture (`src/core`, `src/translators`, `src/cli`)** | 🛠️ Clean, easy to maintain, debug, and extend |
| **Full Feature Parity** | 100% Feature Set | **100% Exact Parity** | ✅ Quota banner, auto proxy detection, profile pooling & token auto-refresh |

---

### 🚀 How to Use the Node.js Edition (วิธีเรียกใช้งาน)

#### 1. Switch to the Node.js Branch
```bash
git fetch origin feat/nodejs-version
git checkout feat/nodejs-version
```

#### 2. Launch the Bridge Server
```bash
# Recommended: create an API key first (without one the server runs in anonymous mode)
node src/index.mjs key generate my-agent

# Direct run (Default Port 8008, Zero Dependencies - No npm install needed!)
node src/index.mjs

# Or run with custom port / options
node src/index.mjs --port 8008 --host 127.0.0.1 --api-key my-secret-key

# Or using npm
npm start
```

#### 3. Run as a Background Service (Linux / macOS / Windows)
You can easily run the Node.js edition as an uninterrupted background daemon using **PM2** or **systemd**:

```bash
# Using PM2 (Recommended for cross-platform):
npm install -g pm2
...
#### 4. Profile & Multi-Agent Key Management CLI
```bash
# View live status of all profiles and quota pools
node src/index.mjs profile list

# Active probe health-check on all profiles
node src/index.mjs profile probe

# Set profile rotation priority order live
node src/index.mjs profile order profile_1,profile_2

# Manage multi-agent API keys
node src/index.mjs key list
node src/index.mjs key generate hermes-agent
```
...
- [⚡ [NEW] Antigravity Bridge (Node.js Edition)](#-new-antigravity-bridge-nodejs-edition---now-available-on-featnodejs-version-)
...
### 4. Manage API Keys (Strongly recommended)
Without a key the server starts in anonymous mode and only warns (see [Security Model](#-security-model)). Generate keys for your AI agents:
```bash
...
```bash
python3 antigravity_bridge.py            # keys come from .env
python3 antigravity_bridge.py --api-key sk-agv-...   # or pass one explicitly
```
> Without any key the server starts in anonymous mode and logs a warning. Never expose that port beyond `127.0.0.1`.
...
# Test HTTP Health endpoint (unauthenticated = liveness only):
curl http://127.0.0.1:8000/health
# Full status (profiles, quota) requires a key:
...
The earlier "Web Extension Edition" (Chrome extension + Docker/noVNC browser sessions used as a
second channel next to the CLI) was removed from the active branches on 2026-09-15. Its full design,
protocol and deployment notes are preserved in [docs/WEB_EXTENSION_BRIDGE_APPROACH.md](docs/WEB_EXTENSION_BRIDGE_APPROACH.md),
and the code is kept as git bundles / tarballs outside the repository (see that document, section 8).

---

## 🔒 Security Model

The bridge runs the `agy` CLI with `--dangerously-skip-permissions`, so **anyone who can reach the port can make the agent run commands and write files on this machine**. Treat the port like SSH access.
...
| agy tool use | **Blocked** | API mode: the model is told not to use agy's own tools; if agy still starts a tool step the run is killed within a second, retried once on the same profile with a reinforced no-tools notice (`ANTIGRAVITY_TOOL_BLOCK_RETRIES`, default `1`), and only then fails with a clear error. `ANTIGRAVITY_ALLOW_CLI_TOOLS=1` re-enables agentic runs. One narrower opt-in exists: `ANTIGRAVITY_ALLOW_TRANSCRIPT_READS=1` lets agy read only the conversation log it writes itself during the run (agy points the model at it when a prompt is too large for one turn). |
...
| **`ANTIGRAVITY_ALLOWED_HOSTS`** | `str` | `None` | Extra `Host` header values to accept besides loopback and the bind host |
...
| **`ANTIGRAVITY_TOOL_BLOCK_RETRIES`** | `int` | `1` | How many times a tool-blocked run is retried on the same profile with a reinforced no-tools notice appended to the prompt. `0` disables. The retry is skipped once text has already been streamed to the client |
| **`ANTIGRAVITY_ALLOW_TRANSCRIPT_READS`** | `int/bool`| `0` | Set `1` to let agy read only its own conversation log for the current run (`<sandbox>/.gemini/antigravity-cli/brain/<conversation>/…`, e.g. `transcript_full.jsonl`). agy does that when a prompt is too large for one turn; blocked, the run dies with `CLI tool execution blocked … view_file …`. Read-only tools, this run's conversation directory only; commands, writes and other conversations stay blocked |
| **`ANTIGRAVITY_MAX_CLI_ARG_BYTES`** | `int` | `120000` | Largest prompt passed as a CLI argument (Linux caps one argument at 131072 bytes). Bigger prompts are sent to agy over stdin as NDJSON (`--input-format stream-json`) up to `ANTIGRAVITY_MAX_STDIN_PROMPT_BYTES` (2 MB) |
| **`ANTIGRAVITY_MAX_PROMPT_CHARS`** | `int` | `200000` | Context budget (UTF-8 bytes) handed to agy. Within budget nothing is truncated; above it older tool results are compacted to `ANTIGRAVITY_OLD_TOOL_OUTPUT_CHARS` (2000) and recent ones to `ANTIGRAVITY_RECENT_TOOL_OUTPUT_CHARS` (20000) |
