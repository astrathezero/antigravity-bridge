...
# Never let the test-suite touch the real ~/.config/antigravity files (bridge_config.json,
# quota cache, sandboxes): point everything at a throw-away directory BEFORE importing the module.
_TEST_STATE_DIR = tempfile.mkdtemp(prefix="agv-bridge-tests-")
os.environ.setdefault("ANTIGRAVITY_BRIDGE_CONFIG", os.path.join(_TEST_STATE_DIR, "bridge_config.json"))
os.environ.setdefault("ANTIGRAVITY_QUOTA_CACHE_FILE", os.path.join(_TEST_STATE_DIR, "quota_cache.json"))
os.environ.setdefault("ANTIGRAVITY_SANDBOX_BASE", os.path.join(_TEST_STATE_DIR, "sandboxes"))
...
class TestAntigravityBridge(unittest.TestCase):
    ...
    def test_server_http_endpoints(self):
        ...
        try:
            ...
            server.shutdown()
            server.server_close()
            # Only the isolated test config (ANTIGRAVITY_BRIDGE_CONFIG) is cleaned up, never the real one.
            test_cfg = antigravity_bridge.get_bridge_config_path()
            if test_cfg.startswith(_TEST_STATE_DIR) and os.path.exists(test_cfg):
                try:
                    os.remove(test_cfg)
                except Exception:
                    pass
    ...
    def test_multi_api_key_server_auth(self):
        ...
        try:
            ...
            # 1. Health check is public but liveness-only when unauthenticated
            with urllib.request.urlopen(urllib.request.Request(health_url)) as resp:
                data = json.loads(resp.read().decode("utf-8"))
                self.assertEqual(data.get("status"), "ok")
                self.assertTrue(data.get("auth_required"))
                self.assertNotIn("active_keys_count", data)
                self.assertNotIn("profiles", data)
            # 1b. Authenticated health check reports full state
            req_health_auth = urllib.request.Request(health_url, headers={"Authorization": "Bearer sk-test-cursor"})
            with urllib.request.urlopen(req_health_auth) as resp:
                data = json.loads(resp.read().decode("utf-8"))
            ...
            # 6. Keys in the query string are NOT accepted (they leak into logs/history)
            req_query = urllib.request.Request(f"{models_url}?api_key=sk-test-cursor")
            with self.assertRaises(urllib.error.HTTPError) as ctx:
                urllib.request.urlopen(req_query)
            self.assertEqual(ctx.exception.code, 401)

            # 6b. Prototype/dunder-style names and near-misses are rejected
            for bad in ("__proto__", "constructor", "sk-test-curso", "sk-test-cursorX"):
                req_bad = urllib.request.Request(models_url, headers={"Authorization": f"Bearer {bad}"})
                with self.assertRaises(urllib.error.HTTPError) as ctx:
                    urllib.request.urlopen(req_bad)
                self.assertEqual(ctx.exception.code, 401)

            # 6c. Foreign Host header is rejected (DNS-rebinding defence)
            req_host = urllib.request.Request(models_url, headers={"Authorization": "Bearer sk-test-cursor", "Host": "evil.example.com"})
            with self.assertRaises(urllib.error.HTTPError) as ctx:
                urllib.request.urlopen(req_host)
            self.assertEqual(ctx.exception.code, 403)

            # 6d. Unsafe profile names and key labels are rejected
            req_cfg = urllib.request.Request(
                f"http://127.0.0.1:{port}/v1/profiles/config",
                data=json.dumps({"profiles": ["../../etc"]}).encode("utf-8"),
                headers={"Authorization": "Bearer sk-test-cursor", "Content-Type": "application/json"},
                method="POST",
            )
            with self.assertRaises(urllib.error.HTTPError) as ctx:
                urllib.request.urlopen(req_cfg)
            self.assertEqual(ctx.exception.code, 400)
            req_key = urllib.request.Request(
                f"http://127.0.0.1:{port}/v1/keys/create",
                data=json.dumps({"label": "x\nANTIGRAVITY_BRIDGE_CMD=evil"}).encode("utf-8"),
                headers={"Authorization": "Bearer sk-test-cursor", "Content-Type": "application/json"},
                method="POST",
            )
            with self.assertRaises(urllib.error.HTTPError) as ctx:
                urllib.request.urlopen(req_key)
            self.assertEqual(ctx.exception.code, 400)
    ...
    def test_calculate_dynamic_stall_timeout(self):
        """Stall threshold is lenient (agy is silent while the model thinks) and does not shrink for short prompts."""
        os.environ.pop("ANTIGRAVITY_STALL_TIMEOUT", None)
        short_stall = antigravity_bridge.calculate_dynamic_stall_timeout(50)
        self.assertGreaterEqual(short_stall, 600.0)

        med_stall = antigravity_bridge.calculate_dynamic_stall_timeout(5000)
        self.assertEqual(med_stall, short_stall)

        long_stall = antigravity_bridge.calculate_dynamic_stall_timeout(80000)
        self.assertEqual(long_stall, short_stall)

        reasoning_stall = antigravity_bridge.calculate_dynamic_stall_timeout(100, model_name="gemini-3.8-flash-high")
        self.assertEqual(reasoning_stall, short_stall)

        # Explicit env override is still honoured
        os.environ["ANTIGRAVITY_STALL_TIMEOUT"] = "45"
        try:
            self.assertEqual(antigravity_bridge.calculate_dynamic_stall_timeout(100, model_name="gemini-3.8-flash-high"), 45.0)
        finally:
            os.environ.pop("ANTIGRAVITY_STALL_TIMEOUT", None)
    ...
    def test_execute_cli_with_fallback_routes_on_stall(self):
        ...
        with patch.object(antigravity_bridge, "execute_cli_command", side_effect=mock_exec):
            ...
            # A stall is usually the model still thinking, not a broken profile: no cooldown applied
            self.assertFalse(pm.is_in_cooldown("hung_profile"))
...
class TestAgyStreamJson(unittest.TestCase):
    def test_parse_stream_json_success(self):
        raw = (
            '{"event":"init","init":{"cwd":"/x"}}\n'
            '{"event":"step_update","step_update":{"step_index":1,"state":"ACTIVE","step_type":"agent_response","text_delta":"17 × 23 = 391"}}\n'
        ...
        out = antigravity_bridge.parse_agy_stream_output(raw)
        self.assertTrue(out.seen_events)
        self.assertEqual(out.deltas, ["17 × 23 = 391", "\n"])
        self.assertEqual(out.final_text(), "17 × 23 = 391\n")
        self.assertFalse(out.is_failure())

    def test_parse_stream_json_failure_and_plain_text(self):
        bad = antigravity_bridge.parse_agy_stream_output('{"event":"result","result":{"status":"ERROR","error":"Individual quota reached. Resets in 3h"}}\n')
        self.assertTrue(bad.is_failure())
        self.assertIn("quota reached", bad.error_text())
        plain = antigravity_bridge.parse_agy_stream_output("plain text answer\n")
        self.assertFalse(plain.seen_events)
        self.assertEqual(plain.final_text(), "plain text answer\n")


class TestApiModeToolGuard(unittest.TestCase):
    def setUp(self):
        self._prev = os.environ.pop("ANTIGRAVITY_ALLOW_CLI_TOOLS", None)

    def tearDown(self):
        if self._prev is not None:
            os.environ["ANTIGRAVITY_ALLOW_CLI_TOOLS"] = self._prev
        else:
            os.environ.pop("ANTIGRAVITY_ALLOW_CLI_TOOLS", None)

    def test_prompt_carries_preamble_unless_allowed(self):
        single = antigravity_bridge.format_messages_to_prompt([{"role": "user", "content": "Hi"}])
        self.assertTrue(single.startswith("[Bridge Mode: API backend]"))
        self.assertTrue(single.endswith("\n\nHi"))
        multi = antigravity_bridge.format_messages_to_prompt([{"role": "system", "content": "S"}, {"role": "user", "content": "U"}])
        self.assertTrue(multi.startswith("[Bridge Mode: API backend]"))
        self.assertIn("[User]\nU", multi)
        os.environ["ANTIGRAVITY_ALLOW_CLI_TOOLS"] = "1"
        self.assertEqual(antigravity_bridge.format_messages_to_prompt([{"role": "user", "content": "Hi"}]), "Hi")

    def test_parser_records_tool_steps(self):
        out = antigravity_bridge.AgyStreamOutput()
        kind, text = out.feed_line('{"event":"step_update","step_update":{"step_index":2,"state":"ACTIVE","step_type":"tool","tool_name":"run_command","tool_info":{"name":"run_command","parameters":{"CommandLine":"ls /"}}}}\n')
    ...
    def test_tool_step_kills_cli_and_is_not_retried_on_other_profiles(self):
        ...
        tpl = f'python3 -c "{script}" {{prompt}}'
        t0 = time.time()
        with self.assertRaises(RuntimeError) as ctx:
            execute_cli_with_fallback(tpl, "hi", timeout=20.0, total_timeout=40.0, profile_manager=pm)
    ...
    def test_cancel_check_kills_cli(self):
        t0 = time.time()
        with self.assertRaises(RuntimeError) as ctx:
            antigravity_bridge.execute_cli_command(
                'python3 -c "import time; time.sleep(30)" {prompt}', "hi", timeout=20.0, profile="zz_t3",
                cancel_check=lambda: time.time() - t0 > 0.7,
            )
        self.assertIn("Client disconnected", str(ctx.exception))
