...
- Secure by default: constant-time API Key auth (loud warning when none configured), Host header
  allow-list (DNS-rebinding defence), CORS off by default, 0600 token files, input bounds checking.
...
# Silence is NOT a hang signal for agy: in print mode it emits nothing while the model is
# thinking (often minutes for reasoning models). The stall watchdog therefore only fires after
# a long quiet period; the hard per-profile timeout remains the real guard.
DEFAULT_STALL_TIMEOUT = float(os.environ.get("ANTIGRAVITY_STALL_TIMEOUT", "600.0"))
...
API_KEY_LABEL_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$")
API_KEY_VALUE_RE = re.compile(r"^[A-Za-z0-9._-]{8,256}$")
_LOOPBACK_HOSTS = ("localhost", "127.0.0.1", "::1", "[::1]")
_WILDCARD_BIND = ("", "0.0.0.0", "::", "[::]", "*")


def is_safe_profile_name(name: Any) -> bool:
    """True when `name` is a safe single path segment (or None/'' meaning the default profile)."""
...
def assert_safe_profile_name(name: Any) -> Any:
    """Raise ValueError if `name` is not a safe profile name (blocks path traversal)."""
    if not is_safe_profile_name(name):
        raise ValueError(f"Invalid profile name: {str(name)[:80]!r}")
    return name


def verify_api_key(candidate: Optional[str], api_keys: Dict[str, str]) -> Optional[str]:
    """Constant-time lookup of `candidate` in the configured key map. Returns the label or None."""
    if not candidate or not isinstance(candidate, str) or not api_keys:
        return None
    matched: Optional[str] = None
    cand_b = candidate.encode("utf-8")
    for key, label in api_keys.items():
        if hmac.compare_digest(cand_b, str(key).encode("utf-8")):
            matched = label or "default"
            # keep iterating so timing is independent of match position
    return matched


def open_private(path: str, mode: str = "w", encoding: Optional[str] = "utf-8"):
    """Open `path` for writing with permissions 0600 (owner read/write only)."""
...
def chmod_private(path: str, is_dir: bool = False) -> None:
    """Best-effort tightening of an existing secret file/dir to 0600/0700."""
    try:
...
def makedirs_private(path: str) -> None:
    """Create a directory tree readable only by the owner."""
    os.makedirs(path, mode=0o700, exist_ok=True)
    chmod_private(path, is_dir=True)


def mac_keychain_store(service: str, account: str, value: str) -> bool:
    """Store a secret in the macOS Keychain via `security -i` (stdin), never via argv."""
    if not re.match(r"^[A-Za-z0-9+/=:_.-]+$", value):
        raise ValueError("Keychain value contains characters unsafe for security -i")
    cmd = f'add-generic-password -U -s {json.dumps(service)} -a {json.dumps(account)} -w {value}\n'
    try:
        res = subprocess.run(
            ["security", "-i"],
        ...
        logger.debug("macOS Keychain store error: %s", e)
        return False


def build_allowed_hosts(bind_host: Optional[str], extra: Optional[str] = None) -> Optional[set]:
    """Set of acceptable Host header values, or None to accept any (wildcard bind, no explicit list)."""
    extra_raw = extra if extra is not None else os.environ.get("ANTIGRAVITY_ALLOWED_HOSTS", "")
    allowed = {h.strip().lower() for h in re.split(r"[\s,]+", str(extra_raw or "")) if h.strip()}
    bind = (bind_host or "").strip().lower()
    if bind in _WILDCARD_BIND and not allowed:
        return None
    allowed.update(_LOOPBACK_HOSTS)
    if bind and bind not in _WILDCARD_BIND:
        allowed.add(bind)
    return allowed


def normalise_host_header(host_header: Optional[str]) -> str:
...
def is_allowed_host(host_header: Optional[str], allowed_hosts: Optional[set]) -> bool:
    if allowed_hosts is None:
        return True
    host = normalise_host_header(host_header)
    return bool(host) and host in allowed_hosts


def sanitize_header_value(value: Any, max_len: int = 512) -> str:
    """Strip CR/LF and control characters so values can never split HTTP headers."""
...
def cli_tools_allowed() -> bool:
    ...
    this host for many minutes on behalf of any API client."""
...
class CLIClientDisconnectedError(RuntimeError):
    """The HTTP client went away; the CLI run was cancelled."""
...
def parse_timeout_value(val: Any) -> Optional[float]:
    ...
    if isinstance(val, (int, float)):
        f_val = float(val)
        if f_val != f_val or f_val in (float("inf"), float("-inf")):
            return None
    ...
    if isinstance(val, str):
        ...
        if not s:
            return None
        # Strip comment/wrapper symbols first: <!-- ... --> or [ ... ]
        s = re.sub(r"^(?:<!--|\[|\{|\(|#|\s|<)+|(?:-->|\]|\}|\)|\s|>)+$", "", s).strip()
        # Strip prefix keywords: timeout=, wait=, profile_timeout:, etc.
        s = re.sub(r"^(?:antigravity[:_])?(?:wait|timeout|profile_timeout|execution_timeout|request_timeout)\s*[=:]\s*", "", s).strip()
        try:
            f_val = float(s)
            if f_val != f_val or f_val in (float("inf"), float("-inf")):
                return None
...
def extract_model_and_timeout(model_name: Optional[str]) -> Tuple[Optional[str], Optional[float]]:
    """Extract clean model name and any embedded timeout specification.

    Supports formats:
    - 'gemini-3.7-flash:timeout=30m' -> ('gemini-3.7-flash', 1800.0)
    - 'gemini-3.7-flash:30m' -> ('gemini-3.7-flash', 1800.0)
    - 'gemini-3.7-flash?timeout=1800' -> ('gemini-3.7-flash', 1800.0)
    - 'gemini-3.7-flash#timeout=20m' -> ('gemini-3.7-flash', 1200.0)
    - 'gemini-3.7-flash/timeout=25m' -> ('gemini-3.7-flash', 1500.0)
    ...
    # 1. Query parameter style: model?timeout=30m or model?profile_timeout=1800
    if "?" in raw:
        base, qs_part = raw.split("?", 1)
        clean_model = base.strip()
        try:
            qs = urllib.parse.parse_qs(qs_part)
            for k in ("timeout", "profile_timeout", "execution_timeout", "request_timeout", "timeout_seconds"):
                if k in qs and qs[k]:
                    timeout_val = parse_timeout_value(qs[k][0])
                    if timeout_val is not None:
                        break
        except Exception:
            pass

    # 2. Fragment style: model#timeout=30m
    elif "#" in raw:
        base, frag = raw.split("#", 1)
        clean_model = base.strip()
        timeout_val = parse_timeout_value(frag)

    # 3. Slash style: model/timeout=30m or model/profile_timeout=30m
    elif "/timeout=" in raw.lower() or "/profile_timeout=" in raw.lower():
        parts = re.split(r"/(?:timeout|profile_timeout)=", raw, flags=re.IGNORECASE, maxsplit=1)
        if len(parts) == 2:
            clean_model = parts[0].strip()
            timeout_val = parse_timeout_value(parts[1].strip())

    # 4. Colon style: model:timeout=30m or model:30m or model:1800s
    elif ":" in raw:
        parts = raw.rsplit(":", 1)
        candidate = parts[1].strip()
        parsed = parse_timeout_value(candidate)
        if parsed is not None:
            clean_model = parts[0].strip()
            timeout_val = parsed

    return clean_model, timeout_val


def extract_timeout_from_prompt_text(prompt_text: str) -> Optional[float]:
    """Extract explicit timeout directive from prompt text or system instructions if present.

    Supports directives:
    - [antigravity:timeout=30m] or [antigravity_timeout=1800]
    - [timeout: 30m] or [profile_timeout=30m]
    - <!-- timeout: 30m --> or <!-- profile_timeout: 1800s -->
    """
    if not prompt_text:
        return None

    # Check first 4000 and last 2000 chars of prompt for speed
    search_sample = prompt_text[:4000] + "\n" + prompt_text[-2000:] if len(prompt_text) > 6000 else prompt_text

    patterns = [
        r"\[(?:antigravity[:_])?(?:profile_)?timeout\s*[:=]\s*([0-9a-zA-Z.:\s]+)\]",
        r"<!--\s*(?:antigravity[:_])?(?:profile_)?timeout\s*[:=]\s*([0-9a-zA-Z.:\s]+)\s*-->",
    ]
    for pat in patterns:
        m = re.search(pat, search_sample, re.IGNORECASE)
        if m:
            val = parse_timeout_value(m.group(1).strip())
...
def calculate_dynamic_stall_timeout(
    ...
    Long-thinking models produce zero output (and near-zero CPU) for minutes; the previous
    35-150s silence-based watchdog killed healthy runs and cascaded into profile fallbacks and
    cooldowns, which clients saw as an endless heartbeat-only stream. One lenient threshold
    (env-overridable via ANTIGRAVITY_STALL_TIMEOUT) is used regardless of prompt size/model.
    """
    ...
    if env_override:
        ...
        if parsed is not None and parsed > 0:
            return parsed
    try:
        base = float(base_stall)
    except Exception:
        base = 600.0
    return base if base > 0 else 600.0
...
def generate_image_via_router(
    ...
    """Generate image using router backend.
    ...
    if not router_url or not router_key:
        raise RuntimeError(
            "Image router is not configured. Please set ANTIGRAVITY_IMAGE_ROUTER_URL and ANTIGRAVITY_IMAGE_ROUTER_KEY in your environment or .env file."
        )
...
def detect_cli_command() -> Tuple[str, str]:
    ...
    if agy_path:
        return ("agy", f'"{agy_path}" --dangerously-skip-permissions --print-timeout 20m0s --output-format stream-json -p "{{prompt}}"')
    ...
    if os.path.exists(local_bin) and os.access(local_bin, os.X_OK if os.name != "nt" else os.F_OK):
        return ("agy", f'"{local_bin}" --dangerously-skip-permissions --print-timeout 20m0s --output-format stream-json -p "{{prompt}}"')

    # Fallback to standard agy command name
    return ("agy", 'agy --dangerously-skip-permissions --print-timeout 20m0s --output-format stream-json -p "{prompt}"')
...
def get_profile_sandbox_base_path(profile_name: Optional[str]) -> str:
    ...
    assert_safe_profile_name(profile_name)
    key = profile_name or "default"
    base = os.environ.get("ANTIGRAVITY_SANDBOX_BASE", "").strip() or os.path.expanduser("~/.config/antigravity/sandboxes")
...
class ProfileManager:
    ...
    def save_cache(self) -> None:
        ...
        try:
            ...
            with open_private(tmp_file) as f:
...
def inject_os_keyring_token(raw_oauth_str: str) -> bool:
    ...
    if os_type == "darwin":
        # Secret is piped over stdin (security -i); never placed in argv where `ps` exposes it.
        return mac_keychain_store("gemini", "antigravity", b64_val)
...
def sync_profile_to_system(profile_name: str) -> Tuple[str, str]:
    ...
    if os.path.exists(oauth_file):
        try:
            ...
            for td in target_dirs:
                try:
                    with open_private(os.path.join(td, "antigravity-oauth-token")) as aot_f:
...
def refresh_profile_token(profile: Optional[str], agy_exec: Optional[str] = None) -> Tuple[bool, str]:
    ...
    assert_safe_profile_name(profile)
    p = profile
    ...
    if sys.platform == "darwin":
        ...
        mac_keychain_store("gemini", "antigravity", b64_val)
        ...
        try:
            ...
            if new_access_tok:
                ...
                with open_private(oauth_file) as f:
                    json.dump(data, f, indent=2)
                with open_private(os.path.join(p_dir, "antigravity-oauth-token")) as f:
        ...
        for td in get_auth_sync_directories():
            makedirs_private(td)
            try:
                with open_private(os.path.join(td, "antigravity-oauth-token")) as a_f:
...
def resolve_model_flags(model_name: Optional[str]) -> List[str]:
    ...
    clean_m, _ = extract_model_and_timeout(model_name)
    model_clean = (clean_m or model_name).strip()
                ...
                for td in target_dirs:
                    try:
                        ...
                        if not os.path.exists(dst_tok) or os.path.getmtime(oauth_file) > os.path.getmtime(dst_tok):
                            with open_private(dst_tok) as aot_f:
                                aot_f.write(tok_json_str)
                    except Exception:
                        pass
            except Exception:
...
class AgyStreamOutput:
    """Parsed view of `agy --output-format stream-json` stdout (NDJSON, one event per line).
    Non-JSON lines are kept verbatim so custom --cmd templates and plain-text output still work."""
    ...
    def feed_line(self, line: str) -> Tuple[str, Optional[str]]:
        ...
        'raw' (non-JSON line) or 'event' (structural event, nothing to forward)."""
        stripped = line.strip()
        if stripped.startswith("{") and '"event"' in stripped:
            ...
            if isinstance(ev, dict) and ev.get("event"):
                ...
                kind = ev.get("event")
                if kind == "step_update" and isinstance(ev.get("step_update"), dict):
                    su = ev["step_update"]
                    ...
                    if su.get("step_type") == "tool" and su.get("state") == "ACTIVE":
                        ...
                        try:
                            params = json.dumps(info.get("parameters", {}), ensure_ascii=False)[:200]
                        except Exception:
                            params = ""
                    ...
                    if isinstance(r.get("response"), str):
                        self.final_response = r["response"]
                    self.error = r.get("error") or r.get("error_message") or r.get("message")
                elif kind == "error":
                    self.status = self.status or "ERROR"
                    self.error = ev.get("error") or ev.get("message") or stripped
                return "event", None
    ...
    def is_failure(self) -> bool:
        return self.seen_events and self.status is not None and self.status != "SUCCESS"
...
def parse_agy_stream_output(raw_stdout: str) -> AgyStreamOutput:
    """Parse a complete stdout capture (used for the non-tracker/mock execution path)."""
    out = AgyStreamOutput()
...
def _invoke_output_callback(callback: Callable[..., None], text: str, kind: str) -> None:
    """Call output_callback(text, kind); fall back to the legacy 1-arg signature."""
    try:
        callback(text, kind)
    except TypeError:
        callback(text)
...
class ProcessActivityTracker:
    ...
    def reader(self, pipe: Any, dest_list: List[str], is_stdout: bool = True) -> None:
        ...
        try:
            for line in iter(pipe.readline, ""):
                ...
                self.record_activity(len(line))  # any stream-json event (init/tool/delta) is activity
                if is_stdout:
                    ...
                    if kind == "tool":
                        if not self.allow_cli_tools:
                            ...
                            logger.info("[TOOL STEP] Profile '%s': %s", self.profile, text)
                    if self.output_callback and text is not None:
                        try:
                            _invoke_output_callback(self.output_callback, text, kind)
                        except Exception:
                            pass
        except Exception:
            pass
    ...
    def run(self, comm_input: Optional[str] = None) -> Tuple[str, str]:
        ...
        while True:
            ...
            # 0. Client gone? Stop burning quota on an answer nobody will read.
            if self.cancel_check is not None:
                try:
                    gone = bool(self.cancel_check())
                except Exception:
                    gone = False
                if gone:
                    self.cancelled = True
                    ...
                    logger.warning("[CLIENT DISCONNECTED] Cancelled CLI execution for profile '%s' after %.1fs", self.profile, elapsed)
                    raise CLIClientDisconnectedError(f"Client disconnected: CLI execution cancelled (profile={self.profile})")
    ...
    with SandboxDirectoryLock(sandbox_base, profile):
        ...
        try:
            ...
            if is_mock or not hasattr(proc.stdout, "fileno"):
                ...
                parsed_stream = parse_agy_stream_output(stdout_data or "")
            else:
        ...
        if proc is not None and proc.returncode != 0:
            err_msg = stderr_data.strip() or (parsed_stream.error_text() if parsed_stream.seen_events else stdout_data.strip()) or f"Exit code {proc.returncode}"
            logger.error("CLI execution failed for profile '%s' (code %d): %s", profile or "default", proc.returncode, err_msg)
            raise RuntimeError(f"CLI Execution Error (profile={profile or 'default'}): {err_msg}")

        output_text = (parsed_stream.final_text().strip() if parsed_stream.seen_events else stdout_data.strip()) or stderr_data.strip()
        if not output_text:
...
def execute_cli_with_fallback(
    ...
    for _ in range(len(candidate_profiles)):
        ...
        if cancel_check is not None:
            try:
                if cancel_check():
                    raise CLIClientDisconnectedError("Client disconnected before CLI execution started")
            except CLIClientDisconnectedError:
                mgr.release_profile(profile)
                raise
            except Exception:
                pass
        try:
            ...
            try:
                sig_params = inspect.signature(execute_cli_command).parameters
            ...
            err_str = str(exc)
            if isinstance(exc, (CLIToolUseBlockedError, CLIClientDisconnectedError)) or "cli tool execution blocked" in err_str.lower() or "client disconnected" in err_str.lower():
                # Retrying on another profile would repeat the same behaviour; no cooldown either.
                logger.warning("Profile '%s': %s (not retrying)", profile_key, exc)
                raise
            ...
            if "sandbox is currently locked" in err_str.lower():
                ...
                logger.warning("[FALLBACK] Profile '%s' produced no output for the stall window. Routing to alternative profile without cooldown.", profile_key)
            else:
...
def generate_image_with_agy(
    ...
    try:
        output_text, used_profile = execute_cli_with_fallback(
            ...
            allow_cli_tools=True,  # this path deliberately asks agy to run a script
        )
    ...
    # Only trust model-reported paths that (a) live in a directory the agent works in and
    # (b) were created during this request; otherwise arbitrary images could be exfiltrated.
    trusted_roots = [os.path.abspath(d) for d in (os.getcwd(), "/tmp", os.path.expanduser("~/.gemini"), os.path.expanduser("~/.config/antigravity/sandboxes"))]
    for p_str in path_candidates:
        clean_p = p_str.strip().strip("'\"()[]<>")
        expanded = os.path.abspath(os.path.expanduser(clean_p))
        if not any(expanded == r or expanded.startswith(r + os.sep) for r in trusted_roots):
            logger.debug("Ignoring model-reported image path outside trusted roots: %s", expanded)
            continue
        try:
            if os.path.getmtime(expanded) < start_ts - 10:
                logger.debug("Ignoring pre-existing image path: %s", expanded)
                continue
        except Exception:
            continue
...
def save_api_key_to_env(label: str, key: str, env_path: Optional[str] = None) -> Tuple[bool, str]:
    ...
    if not API_KEY_LABEL_RE.match(label):
        return False, "Invalid label: use 1-64 chars of A-Z a-z 0-9 . _ -"
    if not API_KEY_VALUE_RE.match(key):
        return False, "Invalid key: use 8-256 chars of A-Z a-z 0-9 . _ -"
...
class AntigravityBridgeHandler(BaseHTTPRequestHandler):
    ...
    def _authorized(self) -> bool:
        ...
        if not api_keys:
            # No API keys configured: anonymous mode (main() logs a loud warning).
        ...
        label = verify_api_key(candidate_key, api_keys)
        if label is not None:
            self.auth_client = label
            return True

        return False

    def _client_disconnected(self) -> bool:
        """True when the HTTP client has closed its connection (heartbeat write failed or socket at EOF)."""
        hb = getattr(self, "_heartbeat", None)
        if hb is not None and getattr(hb, "client_gone", False):
            return True
        flag = getattr(socket, "MSG_DONTWAIT", None)
        if flag is None:
            return False
        try:
            data = self.connection.recv(1, socket.MSG_PEEK | flag)
            return data == b""
    ...
    def _auth_configured(self) -> bool:
        return bool(getattr(self.server, "api_keys", None) or getattr(self.server, "api_key", None))

    def _host_allowed(self) -> bool:
        """Reject requests whose Host header is not on the allow-list (DNS-rebinding defence)."""
        allowed = getattr(self.server, "allowed_hosts", "__unset__")
        if allowed == "__unset__":
            try:
                bind_host = self.server.server_address[0]
            except Exception:
                bind_host = "127.0.0.1"
            allowed = build_allowed_hosts(bind_host)
            self.server.allowed_hosts = allowed
        if is_allowed_host(self.headers.get("Host"), allowed):
            return True
        ...
        try:
            ...
            if extra_headers:
                for hk, hv in extra_headers.items():
                    self.send_header(hk, sanitize_header_value(hv))
    ...
    def _send_cors_headers(self) -> None:
        if not getattr(self.server, "enable_cors", False):
            self._send_json_response({"error": {"message": "CORS is disabled on this bridge (start with --enable-cors to allow browser clients)", "type": "forbidden", "code": 403}}, status_code=403)
            return
    ...
    def _extract_request_timeouts(
        ...
        for h_key in (
            ...
            "OpenAI-Timeout", "X-Server-Timeout", "X-Gateway-Timeout",
            "X-Client-Timeout", "X-Model-Timeout", "X-Timeout-Seconds",
        ...
        # Check URL Query Parameters (priority 2)
        if req_prof_timeout is None and hasattr(self, "path") and self.path:
            try:
                ...
                for q_key in (
                    ...
                    "timeout_seconds", "timeout_sec", "max_execution_time", "max_timeout",
                    "http_timeout", "read_timeout", "client_timeout",
                ):
        ...
        # Check Request Body (priority 3)
        if req_prof_timeout is None and isinstance(req_json, dict):
            for b_key in (
                "profile_timeout", "execution_timeout", "timeout", "request_timeout",
                "timeout_seconds", "timeout_sec", "max_execution_time", "max_timeout",
                "http_timeout", "read_timeout", "client_timeout", "timeout_ms",
            ):
                ...
                for container_key in (
                    "extra_body", "options", "metadata", "params", "request_options",
                    "config", "settings", "custom_body", "custom_params", "model_kwargs",
                    "kwargs", "payload", "properties", "generation_config", "system_config",
                ):
        ...
        # Check Model Embedded Timeout (priority 4)
        if req_prof_timeout is None and model_timeout is not None and model_timeout > 0:
            req_prof_timeout = model_timeout

        # Check In-Prompt Directives (priority 5)
        if req_prof_timeout is None and raw_prompt_text:
            in_prompt = extract_timeout_from_prompt_text(raw_prompt_text)
            if in_prompt is not None and in_prompt > 0:
                req_prof_timeout = in_prompt
        ...
        for h_key in (
            "X-Total-Timeout", "X-Fallback-Timeout", "X-Total-Fallback-Timeout",
            "X-Max-Total-Timeout", "Total-Timeout",
        ):
            h_val = self.headers.get(h_key)
            if h_val:
                parsed = parse_timeout_value(h_val)
        ...
        if req_total_timeout is None and hasattr(self, "path") and self.path:
            try:
                parsed_url = urllib.parse.urlparse(self.path)
                qs = urllib.parse.parse_qs(parsed_url.query)
                for q_key in ("total_timeout", "fallback_timeout", "total_fallback_timeout", "max_total_timeout"):
                    if q_key in qs and qs[q_key]:
                        parsed = parse_timeout_value(qs[q_key][0])
                        if parsed is not None and parsed > 0:
                            req_total_timeout = parsed
        ...
        if req_total_timeout is None and isinstance(req_json, dict):
            for b_key in ("total_timeout", "fallback_timeout", "total_fallback_timeout", "max_total_timeout"):
                b_val = req_json.get(b_key)
                if b_val is not None:
                    parsed = parse_timeout_value(b_val)
                    if parsed is not None and parsed > 0:
    ...
    def _extract_stall_timeout(
        ...
        # Client-supplied stall timeouts are clamped so a request can never pin a profile forever.
        raw_cap = getattr(self.server, "max_total_timeout", None)
        stall_cap = float(raw_cap) if isinstance(raw_cap, (int, float)) and raw_cap > 0 else DEFAULT_MAX_TOTAL_TIMEOUT
        for h_key in ("X-Stall-Timeout", "X-Inactivity-Timeout", "Stall-Timeout", "X-Antigravity-Stall-Timeout"):
            h_val = self.headers.get(h_key)
            if h_val:
                parsed = parse_timeout_value(h_val)
                if parsed is not None and parsed > 0:
                    return min(parsed, stall_cap)
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path in ("", "/health"):
                if self._auth_configured() and not self._authorized():
                    # Liveness only: never disclose profile names, errors or key counts anonymously.
                    self._send_json_response({"status": "ok", "service": "antigravity-bridge", "auth_required": True})
                    return
    ...
    def do_POST(self) -> None:
        try:
            ...
            if content_length > 0:
                ...
                req_json = {}
            if is_keys_create:
                label = str(req_json.get("label") or "agent-custom").strip()
                custom_key = str(req_json.get("key") or "").strip()
                if not API_KEY_LABEL_RE.match(label):
                    self._send_json_response({"error": {"message": "Invalid label: use 1-64 chars of A-Z a-z 0-9 . _ -", "type": "invalid_request_error"}}, status_code=400)
                    return
                if custom_key and not API_KEY_VALUE_RE.match(custom_key):
                    self._send_json_response({"error": {"message": "Invalid key: use 8-256 chars of A-Z a-z 0-9 . _ -", "type": "invalid_request_error"}}, status_code=400)
                    return
            ...
            if is_keys_revoke:
                ...
                if not (API_KEY_LABEL_RE.match(target_str) or API_KEY_VALUE_RE.match(target_str)):
                    self._send_json_response({"error": "Invalid target"}, status_code=400)
                    return
                ok, path_or_err, removed = revoke_api_key_from_env(target_str)
                if ok:
                    new_keys = get_configured_api_keys()
                    if hasattr(self.server, "api_keys"):
                        self.server.api_keys = new_keys
            ...
            if is_profiles_config:
                ...
                if raw_p is not None:
                    ...
                    bad = [p for p in new_profiles if not is_safe_profile_name(p)]
                    if bad:
                        self._send_json_response({"error": {"message": f"Invalid profile name: {str(bad[0])[:80]!r}", "type": "invalid_request_error"}}, status_code=400)
                        return
            ...
            if is_profiles_disable:
                ...
                if target_p and not is_safe_profile_name(target_p):
                    self._send_json_response({"error": "Invalid profile name"}, status_code=400)
                    return
                if target_p:
            ...
            if is_image_gen:
                ...
                if not prompt:
                    ...
                    return
                raw_img_model = req_json.get("model") or "gemini-3.1-flash-image"
                model_name, img_model_timeout = extract_model_and_timeout(raw_img_model)
                img_timeout, _ = self._extract_request_timeouts(
                    req_json,
                    prompt_len=len(prompt),
                    raw_prompt_text=prompt,
                    model_timeout=img_model_timeout,
                )
                try:
            ...
            raw_model = req_json.get("model") or "antigravity"
            clean_model, model_timeout = extract_model_and_timeout(raw_model)
            model = clean_model or "antigravity"
            ...
            prof_timeout, total_timeout = self._extract_request_timeouts(
                req_json,
                prompt_len=len(prompt_text),
                raw_prompt_text=prompt_text,
                model_timeout=model_timeout,
            ...
            if req_profile and not is_safe_profile_name(str(req_profile)):
                req_profile = None  # ignore unsafe / malformed profile hints
            ...
            live_streamed: List[str] = []
            live_output_callback: Optional[Callable[..., None]] = None
            if stream and heartbeat is not None and not is_anthropic and not normalized_tools:
                def live_output_callback(text: str, kind: str = "raw") -> None:
                    if kind != "delta" or not text:
                        return
                    first = not live_streamed
                    live_streamed.append(text)
                    delta_obj: Dict[str, Any] = {"content": text}
                    if first:
            ...
            if stream:
                if parsed_tool_calls:
                    ...
                    remaining = final_text_content
                    streamed_trim = "".join(live_streamed).strip()
                    if streamed_trim and remaining.startswith(streamed_trim):
                        remaining = remaining[len(streamed_trim):]
                    start_delta: Dict[str, Any] = {"content": remaining}
...
""")
    ...
    if sub in ("list", "ls", "status"):
        ...
        try:
            ...
            if keyring_data:
                ...
                if token_info and token_info.get("access_token"):
                    ...
                    with open_private(os.path.join(target_dir, "oauth_creds.json")) as f:
                        json.dump(oauth_data, f, indent=2)
                    with open_private(os.path.join(target_dir, "antigravity-oauth-token")) as f:
            ...
            if verified_email:
                with open_private(os.path.join(target_dir, "google_accounts.json")) as f:
                    json.dump({"active": verified_email, "old": []}, f, indent=2)
                with open_private(os.path.join(target_dir, "state.json")) as f:
        ...
        name = argv[1].strip()
        remote = argv[2].strip()
        if not is_safe_profile_name(name) or not name:
            print(f"[Error] Invalid profile name {name!r}: use letters, digits, '.', '_' or '-' only")
            return 1
        if not re.match(r"^[A-Za-z0-9._@-]+$", remote):
            print(f"[Error] Invalid remote {remote!r}: expected [user@]host")
            return 1
...
def main():
    # 0. Every file this process creates (tokens, sandboxes, .env, logs) is owner-only.
    try:
        os.umask(0o077)
    except Exception:
        pass
    ...
    parser.add_argument("--enable-cors", "--cors", action="store_true", help="Enable wildcard CORS headers (Access-Control-Allow-Origin: *). OFF by default: with CORS on, any web page you visit can call this bridge.")
    parser.add_argument("--allowed-hosts", default=os.environ.get("ANTIGRAVITY_ALLOWED_HOSTS", ""), help="Comma-separated extra Host header values to accept (loopback + bind host are always allowed; a 0.0.0.0 bind accepts any Host unless this is set)")
    ...
    if not configured_api_keys:
        logger.warning(
            "No API key configured: every endpoint is unauthenticated (anonymous mode). "
            "Anyone who can reach this port can drive the agy agent on this machine. "
            "Keep the bind on 127.0.0.1 and create a key with: python3 antigravity_bridge.py key generate my-agent"
        )
    if args.enable_cors:
        logger.warning("CORS enabled with wildcard origin: any web page you open can call this bridge.")
    ...
    server.allowed_hosts = build_allowed_hosts(args.host, args.allowed_hosts)
    ...
    if server.api_keys:
        ...
        logger.warning("API Key Authentication: DISABLED (anonymous mode - localhost only!)")
    logger.info("Host allow-list:    %s", "any (wildcard bind)" if server.allowed_hosts is None else ", ".join(sorted(server.allowed_hosts)))
    logger.info("CORS:               %s", "ENABLED (wildcard origin)" if args.enable_cors else "disabled")
