...
| Quota-exhausted profile | Fast fail | **Node.js only.** When an account's quota is used up (`RESOURCE_EXHAUSTED (code 429): Individual quota reached … Resets in 64h`), agy retries the call in place for ~2.5 minutes although no retry can succeed. The bridge reads the error text from the run's transcript as soon as agy logs an error step, ends the run within seconds (`[QUOTA]` in the journal), puts the profile in cooldown until the reset and moves on to the next profile (`[FALLBACK]`, one line per switch). `ANTIGRAVITY_QUOTA_FAST_FAIL=0` disables. Idle profiles are rotated least-recently-used so quota use spreads over every account (`ANTIGRAVITY_PROFILE_SELECTION=ordered` restores configuration order). |
...
| **`ANTIGRAVITY_QUOTA_FAST_FAIL`** | `1` | **Node.js only.** End an agy run within seconds once its transcript shows a hard quota error (`Individual quota reached … Resets in …`) instead of waiting ~2.5 min for agy's own retries; the profile cools down until the reset. `0` disables. |
| **`ANTIGRAVITY_PROFILE_SELECTION`** | `lru` | **Node.js only.** Which idle profile takes the next request: `lru` = the one unused longest (spreads quota use over all accounts; a profile pinned with `ANTIGRAVITY_PROFILE` still goes first), `ordered` = the first idle profile in configuration order. A profile in a short error back-off is tried last in both modes. |
